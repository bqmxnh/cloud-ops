...
# ============================================================
def parse_args():
    ap = argparse.ArgumentParser("Incremental Retrain HAT (Production → Staging)")
...
def main():
    ...
    print(f"✔ HAT model loaded from Production")
    ...
    # --------------------------------------------------------
    # INCREMENTAL TRAINING (HAT learns incrementally and adapts)
    # --------------------------------------------------------
    ...
    # MLFLOW LOGGING
    # --------------------------------------------------------
    with mlflow.start_run(run_name="hat-incremental-retrain") as run:
        ...
        mlflow.log_param("algorithm", "HAT")
    ...
    if PROMOTE:
        ...
        s3.put_object(
            ...
            Key="cooldown/last_retrain_ts_hat.txt",
            Body=ts.encode("utf-8")
        )
        print(f"\n[STATE] last_retrain_ts_hat updated = {ts}")
